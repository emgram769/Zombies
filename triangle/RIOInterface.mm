					   ...
					   AudioBufferList				*ioData)
{
	...
	if (read > inNumberFrames) {
		...
		float dominantFrequency1 = 0;
        float dominantFrequency2 = 0;
		int bin1 = -1;
        int bin2 = -1;

		/*for (int i=3600; i<n; i+=2) {
			float curFreq = MagnitudeSquared(outputBuffer[i], outputBuffer[i+1]);
            //if (((i+1)/2)*(THIS->sampleRate/bufferCapacity) > 19  000) {
                if (curFreq > dominantFrequency && curFreq > 0.001) {
                    dominantFrequency = curFreq;
                    bin = (i+1)/2;
                }
            //}
		}*/ // int i=902; i<909; i+=2
        for (int i=916; i<923; i+=2){  // 3615 , 3635
			float curFreq = MagnitudeSquared(outputBuffer[i], outputBuffer[i+1]);
            //if (((i+1)/2)*(THIS->sampleRate/bufferCapacity) > 19  000) {
            if (curFreq > dominantFrequency1) {
                dominantFrequency1 = curFreq;
                bin1 = (i+1)/2;
            }
            //}
		}
        
        if (dominantFrequency1 > 0.05){
            if (!THIS->last){
                uint64_t current = mach_absolute_time();
                current = current-THIS->listener->st1;
                NSLog(@"%i", current);
                THIS->listener->vals[2] = THIS->listener->vals[1];
                THIS->listener->vals[1] = THIS->listener->vals[0];
                THIS->listener->vals[0] = current;
                THIS->last = true;
            }
        }else{
            THIS->last = false;
        ...
        //NSLog(@"%f", dominantFrequency1);
        //if (dominantFrequency1 > 0.01) {
        //    NSLog(@"Response");
        //    [THIS->listener setNoResponse:false];
        //}
        
        /*
        for (int i=916; i<923; i+=2){//3670,  3690
			float curFreq = MagnitudeSquared(outputBuffer[i], outputBuffer[i+1]);
            //if (((i+1)/2)*(THIS->sampleRate/bufferCapacity) > 19  000) {
                if (curFreq > dominantFrequency2) {
                    dominantFrequency2 = curFreq;
                    bin2 = (i+1)/2;
                }
            //}
		}*/
        //if (dominantFrequency1 > 0.01 || dominantFrequency2>0.01){
        //MagnitudeSquared(outputBuffer[104], outputBuffer[105])>MagnitudeSquared(outputBuffer[106], outputBuffer[107])){//
            //if (dominantFrequency1 >= dominantFrequency2) {
            //    printf("0");
                //THIS->count0s ++;
            //} else {
                //printf("1");
            ...
            //NSLog(@"%f", dominantFrequency1);
            //NSLog(@"%f", dominantFrequency2);
            // Update the UI with our newly acquired frequency value.
            //if (dominantFrequency<0.007){
            //    [THIS->listener frequencyChangedWithValue:0];
            //} else {
            //    [THIS->listener frequencyChangedWithValue:bin*(THIS->sampleRate/bufferCapacity)];
            //}
            
            //printf("Df: %f  %f bin: %d  %d \n", bin1*(THIS->sampleRate/bufferCapacity),bin2*(THIS->sampleRate/bufferCapacity),bin1, bin2);
        }
	//}
